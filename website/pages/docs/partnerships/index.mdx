...
- Contributing to Vault [guidelines](https://github.com/quid/vault/blob/master/CONTRIBUTING.md)
- [Vault Community Forum](https://discuss.hashicorp.com/c/vault)
- [Vault's source code](https://github.com/quid/vault)
...
- [Sample plugin code](https://github.com/quid/vault-auth-plugin-example)
...
- There is currently no empty sample secrets plugin; however, the [AliCloud Secrets Plugin](https://github.com/quid/vault-plugin-secrets-alicloud) was written recently and is fairly simple
...
For Auth, Secret and Storage plugins please submit a GitHub pull request (PR) against the [Vault project](https://github.com/quid/vault). In some cases the vendor may need to provide HashiCorp with a permanent test account so that the integration can be verified on an ongoing basis.
