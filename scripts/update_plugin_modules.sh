...
## Get Vault
echo "Fetching vault..."
git clone https://github.com/quid/vault

for plugin in $(grep github.com/quid/vault-plugin- vault/go.mod | cut -f 2 | cut -d ' ' -f 1 | cut -d '/' -f 3)
do
	...
	then
		...
		cd $plugin
		rm -rf vendor
		go get github.com/quid/vault/api${API_BRANCH}
		go mod tidy
		go mod vendor
		git add .
